...
export function BackupList({ onSelect }: BackupListProps) {
  const { backups, loading, error } = useBackupList();
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 text-text-secondary">
        ...
        <p className="text-[11px] text-text-muted">
          Set{" "}
          <code className="font-mono bg-bg-tertiary px-1.5 py-0.5 rounded">
            SPECTRAL_BACKUP_DIR
          </code>{" "}
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 text-text-secondary">
        ...
        <p className="text-[12px] text-text-muted max-w-[400px] text-center">
          Run <strong>"Spectral: Backup Now"</strong> from the Command Palette in Antigravity, then
          reload this page.
        </p>
      </div>
    );
